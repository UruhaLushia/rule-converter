...
pub fn list_geosite_codes_from_buffer(input: Uint8Array) -> Result<Vec<String>> {
    list_geosite_codes_from_bytes(input.as_ref()).map_err(to_napi_error)
}
...
pub fn list_geosite_codes(input: String) -> Result<Vec<String>> {
    ...
    list_geosite_codes_from_bytes(&bytes).map_err(to_napi_error)
}

fn list_geosite_codes_from_bytes(input: &[u8]) -> anyhow::Result<Vec<String>> {
    list_geosite_dat_codes(input).or_else(|_| list_sing_geosite_codes(input))
}
